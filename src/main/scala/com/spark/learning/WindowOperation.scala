...
object WindowOperation extends App {
 ...
 Logger.getLogger("org").setLevel(Level.ERROR)
  ...
  val lagData = customer1.withColumn("prevAmountSpent", lag(customer1("amountSpent"), 1).over(wSpec3))
   val leadData = customer1.withColumn("prevAmountSpent", lead(customer1("amountSpent"), 1).over(wSpec3))
 ...
 // leadData.show()
  ...
  val testData2 = rankData1.withColumn("Page_url", concat_ws("_", rankData1.columns.map(c => col(c)): _*))
  rankData1.show()
  testData2.show()
}